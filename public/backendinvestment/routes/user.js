...
router.post('/deposit-proof', protect, upload.single('proofImage'), async (req, res) => {
    try {
        ...
        const depositProof = await DepositProof.create({
            ...
            proofImageUrl: req.file ? `/uploads/proofs/${req.file.filename}` : null,
            notes: notes || '',
            status: 'pending'
        });
...
router.get('/admin/withdrawal-requests', protect, async (req, res) => {
    try {
...
router.post('/admin/withdraw-request/:id/approve', protect, async (req, res) => {
    try {
        // Only admin can access this
        const adminEmails = ['admin@globalvest.com', 'support@globalvest.com'];
        if (!adminEmails.includes(req.user.email)) {
        ...
        if (user.buyingPower < transaction.amountUSD) {
            return res.status(400).json({ message: 'Insufficient user funds' });
        }
        
        user.buyingPower -= transaction.amountUSD;
        user.totalPortfolioValue -= transaction.amountUSD;
        await user.save();
...
router.post('/admin/withdraw-request/:id/reject', protect, async (req, res) => {
    try {
...
router.post('/send-money', protect, async (req, res) => {
    try {
        ...
        // Add to recipient
        recipient.buyingPower += amount;
        recipient.totalPortfolioValue += amount;
        ...
        res.json({
            ...
            newBalance: req.user.buyingPower,
            transaction: {
...
router.post('/upgrade-request', protect, async (req, res) => {
    try {
        const { requestedTier, reason, currentTier } = req.body;
