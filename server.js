...
// ÔöÇÔöÇ PERSISTENT MOCK STORAGE MANAGER FOR SERVERLESS DEPLOYMENTS ÔöÇÔöÇ
    ...
    dbLoadingPromise = (async () => {
        ...
        for (let attempt = 1; attempt <= maxAttempts; attempt++) {
            try {
                ...
                lastError = err;
                console.warn(`ÔÜá´©Å [Attempt ${attempt}/${maxAttempts}] Database load failed: ${err.message}`);
        ...
        console.error('ÔØî Critical: Failed to load database from JsonBlob after all attempts. Falling back to current in-memory database state.', lastError);
    })().finally(() => {
...
const modifyDb = async (fn) => {
    try {
        ...
        console.log('­ƒÆ¥ DB updated and persisted to JsonBlob.');
    } catch (e) {
...
.catch(err => {
    console.warn('ÔÜá´©Å Cloud MongoDB Connection Failed. Enabling In-Memory Mock Storage Mode for LEAF PRO SSO...');
    ...
    const makeMockModel = (ModelClass, collectionName, defaultProps = {}) => {
        ...
        const mockQueryChain = (result) => {
            ...
            promise.limit = function(num) {
                if (Array.isArray(result) && num) {
                    result = result.slice(0, parseInt(num));
                }
                return mockQueryChain(result);
            };
            promise.skip = function(num) {
                if (Array.isArray(result) && num) {
                    result = result.slice(parseInt(num));
                }
                return mockQueryChain(result);
            };
            promise.populate = function(field, select) {
                const doPopulate = (item) => {
                    ...
                    if (refId) {
                        ...
                        const found = global.MOCK_DB[collection]?.find(u => u._id?.toString() === refId);
                        if (found) {
                            item[field] = { ...found };
                        }
                    }
